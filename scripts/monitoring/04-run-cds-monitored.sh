...
port="$(port_for_mode cds)"
log_file="${LOG_DIR}/monitoring-cds.log"
start_standard_mode cds "${port}" "${log_file}" >/dev/null
pid="$(cat "${PID_DIR}/cds.pid")"
warm_gateway_endpoints "${port}"

cat <<MSG
CDS gateway is running.
PID: ${pid}
URL: http://localhost:${port}
Metrics: http://localhost:${port}/actuator/prometheus
Log: ${log_file}

Prometheus target: gateway-cds
