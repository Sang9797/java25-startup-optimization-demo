...
LONG_SAMPLES_CSV="${LONG_RESULTS_DIR}/baseline-vs-native-samples.csv"
LONG_SUMMARY_CSV="${LONG_RESULTS_DIR}/baseline-vs-native-summary.csv"
LONG_SUMMARY_MD="${LONG_RESULTS_DIR}/baseline-vs-native-summary.md"
...
run_mode() {
  ...
  if [[ "${mode}" == "baseline" ]]; then
    start_standard_mode baseline "${port}" "${startup_log}" >/dev/null
  else
...
write_markdown_summary() {
  {
    echo "# Long Run Baseline vs Native"
    echo
    ...
    awk -F, '
      ...
      END {
        if (!("baseline" in count) || !("native" in count)) {
        ...
        print "Verdict:"
        if (throughput["native"] >= throughput["baseline"]) {
          printf "- Native throughput is %.2f%% of baseline or better.\n", throughput["native"] / throughput["baseline"] * 100
        } else {
          printf "- Native throughput is %.2f%% of baseline.\n", throughput["native"] / throughput["baseline"] * 100
        }
        if (p95["native"] <= p95["baseline"]) {
          printf "- Native p95 latency is %.2f%% of baseline or better.\n", p95["native"] / p95["baseline"] * 100
        } else {
          printf "- Native p95 latency is %.2f%% of baseline.\n", p95["native"] / p95["baseline"] * 100
        }
        if (rss["native"] <= rss["baseline"]) {
          printf "- Native average RSS is %.2f%% of baseline or better.\n", rss["native"] / rss["baseline"] * 100
        } else {
...
cat <<MSG
Long-run baseline vs native benchmark complete.
...
Use longer settings for a stronger steady-state signal, for example:
  LONG_WARMUP_DURATION=5m LONG_DURATION=30m LONG_K6_VUS=16 scripts/native/06-long-run-baseline-vs-native.sh
MSG
