...
LONG_RESULTS_DIR="${LOG_DIR}/long-run-grafana-native-vs-leyden-aot"
mkdir -p "${LONG_RESULTS_DIR}"

leyden_port="$(port_for_mode leyden-aot)"
native_port="$(port_for_mode native)"
leyden_log="${LONG_RESULTS_DIR}/leyden-aot-startup.log"
native_log="${LONG_RESULTS_DIR}/native-startup.log"
leyden_k6_summary="${LONG_RESULTS_DIR}/leyden-aot-k6.json"
native_k6_summary="${LONG_RESULTS_DIR}/native-k6.json"
leyden_k6_log="${LONG_RESULTS_DIR}/leyden-aot-k6.log"
...
K6_BASE_URL="http://127.0.0.1:${leyden_port}" k6 run \
  --vus "${LONG_K6_VUS}" \
  --duration "${LONG_DURATION}" \
  --summary-export "${leyden_k6_summary}" \
